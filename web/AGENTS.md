...
- **Brand**: navy `#152341` and gold `#CFA42D` from the logo, stepped into `navy-50…950` / `gold-50…950` in `globals.css`. Navy carries the interface (it is `--primary`); gold is the accent that marks state — and never small text on a light surface (2.3:1). Reach for the semantic tokens first; the ramps are for where the brand itself is the point. `<ToneBadge>` tones stay functional, not brand.
- **Logo**: `<BrandGlyph>` (globe only — the sole cut legible under ~64px) and `<BrandLockup>` (full, needs ~120px height) from `@/components/brand-logo`; both swap to a white-ink variant on dark. Never re-crop the master by hand.
